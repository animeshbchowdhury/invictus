...
from mcts import execute_episode,test_episode,embed_test_episode
import argparse,os
...
def main():
    ...
    def test_network():
        ...
        listOfEmbeddings = embed_test_episode(trainer,simulationBudget,csvResultFile,LogicSynthesisEnv(origAIG=destRootAIGPath,logFile=logFilePath,libFile=libraryPath))
        return listOfEmbeddings
    
    designEmbeddings = test_network()
    designNameEmbedList = [benchmarkDesignName,designEmbeddings]
    print(designNameEmbedList)
    embeddingPickleFile = osp.join(rootDumpDir,benchmarkDesignName+"_"+str(runID)+".pkl")
    with open(embeddingPickleFile,'wb') as f:
        pickle.dump(designNameEmbedList,f)
